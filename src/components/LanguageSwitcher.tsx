...
export function LanguageSwitcher() {
    ...
    // Only set lang attribute for SEO — dir is handled by LandingPageWrapper
    useEffect(() => {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent
                align="end"
                className="w-48 bg-gradient-to-b from-[#1c1c1c] to-[#2e2e2e] border border-[#404040]"
            >
                {LANGUAGES.map(({ code, label }) => (
                    <DropdownMenuItem
                        key={code}
                        onClick={() => changeLanguage(code)}
                        className={`flex items-center justify-between px-4 py-2 cursor-pointer transition-all duration-200 ${currentLang === code
                                ? 'bg-cyan-400/10 text-cyan-400'
                                : 'text-gray-300 hover:text-white hover:bg-[#404040]'
                            }`}
                    >
                        <span className="font-medium">{label}</span>
                        {currentLang === code && <Check className="w-4 h-4 text-cyan-400" />}
                    </DropdownMenuItem>
                ))}
            </DropdownMenuContent>
        </DropdownMenu>
