...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (inputValue.toLowerCase().includes("strategy") || inputValue.toLowerCase().includes("startup")) {
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: "I can help with that! What specific aspects of this topic would you like to explore further?",
          },
        ])
      }
    }, 1000)
  ...
  return (
    <div className="flex h-screen bg-[#1B1B1B] text-white">
      ...
      <div className="flex-1 flex flex-col max-w-5xl mx-auto h-full relative">
        {/* Mobile menu toggle */}
        <div className="md:hidden absolute top-4 left-4 z-10" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
        ...
        {showWelcomeMessage ? (
          <div className="flex-1 flex items-center justify-center p-4">
            <div className="text-center max-w-4xl">
              <p className="text-xl lg:text-3xl font-normal text-[#62C1BF]">Ask me anything about your course, crypto terms, or how Web3 works</p>
            </div>
          </div>
        ) : (
        ...
        {/* Input */}
        <div className="p-4 border-t  border-[#006A82]">
          <form onSubmit={handleSubmit} className="relative">
            <input
              ...
              className="w-full text-white bg-gradient-to-b z-10  from-[#1A1A1A] via-[#1A1A1A] to-[#3f3d3d] placeholder-[#62C1BF] py-10 rounded-2xl border border-[#62C1BF]  pl-4 pr-12 focus:outline-none focus:ring-2 focus:ring-[#33CDF0]"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-[#33CDF0] text-[#005163] hover:bg-opacity-80 transition-colors"
            >
