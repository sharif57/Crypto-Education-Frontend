...
//                     className={`px-3 py-1.5 text-sm font-medium transition-colors rounded-sm ${activeTab === 'videos'
//                       ? 'text-[#62C1BF] border-b-2 border-[#62C1BF]'
...
//                     className={`px-3 py-1.5 text-sm font-medium transition-colors rounded-sm ${activeTab === 'description'
//                       ? 'text-[#62C1BF] border-b-2 border-[#62C1BF]'
//                       : 'text-gray-400 hover:text-white'
//                       }`}
//                   >
...
export default function VideoDetailPage() {
  ...
  return (
    <div className="w-full bg-gradient-to-b from-[#326866] to-[#1B1B1B] pb-[300px]">
      <div className="container pt-[100px] lg:pt-[120px] mx-auto lg:px-8 px-4 text-white relative">
        {/* Header */}
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 p-3 sm:p-4">
          <Button
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="lg:flex items-start justify-between gap-8">
            <div className="flex-1">
              ...
              {/* ── Tab Bar ── */}
              <div className="text-text flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-2 border-b-2 border-[#7D7D7D] mt-[24px] pb-[12px]">


                {/* Tabs */}
                <div className="flex items-end gap-0 mb-2 lg:-mb-[17px]">
                  <button
                    onClick={() => setActiveTab('videos')}
                    className={`px-4 pb-2 text-sm  lg:text-[24px] font-nomal transition-colors border-b-2 ${activeTab === 'videos'
                      ? 'text-[#62C1BF] border-[#62C1BF]'
                      : 'text-[#999999] hover:text-white border-transparent'
                      }`}
                  >
                    Videos
                  </button>
                  <button
                    onClick={() => setActiveTab('description')}
                    className={`px-4 pb-2 text-sm  lg:text-[24px] font-nomal transition-colors border-b-2 ${activeTab === 'description'
                      ? 'text-[#62C1BF] border-[#62C1BF]'
                      : 'text-[#999999] hover:text-white border-transparent'
                      }`}
                  >
              ...
              <div className="mt-6">
                ...
                {activeTab === 'videos' && (
                  <div className="space-y-3 w-full lg:w-2/3">
                    <h2 className="text-xl font-medium text-white mb-4">Course Modules</h2>
                    <div
                      className={`bg-[#2a2a2a] rounded-xl overflow-hidden border transition-colors ${expandedModule === 'related-videos' ? 'border-[#62C1BF]' : 'border-[#3a3a3a]'
                        }`}
                    >
                      <button
                        onClick={() => toggleModule('related-videos')}
                        ...
                        <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <div className="text-left min-w-0">
                            <h3 className="text-white font-medium text-sm sm:text-base">
                              {data?.data?.category_name || "Up Next Videos"}
                            </h3>
                            <p className="text-[#62C1BF] text-xs sm:text-sm">
                              {data.related_videos?.length || 0} Video{data.related_videos?.length !== 1 ? 's' : ''}
                            </p>
                      ...
                      {expandedModule === 'related-videos' && (
                        <div className="px-3 sm:px-4 pb-4 space-y-2 border-t border-[#3a3a3a]">
                          {sortedRelatedVideos?.map((video: RelatedVideo) => (
                            ...
                            >
                              <div
                                className={`w-7 sm:w-8 h-7 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0 ${video.progress?.is_completed ? 'bg-green-600' : 'bg-[#62C1BF]'
                                  }`}
                              >
                                {video.progress?.is_completed ? (
                                  <span className="text-white text-sm">✓</span>
                              ...
                              <p className="text-xs text-green-500">
                                {video?.progress?.is_completed ? 'already completed' : ''}
                              </p>
                            </button>
                          ))}
                        </div>
                ...
                {activeTab === 'description' && (
                  <div className="w-full lg:w-2/3">
                    {video?.video_description ? (
                      ...
                      <p className="text-gray-400 text-sm">No description available for this video.</p>
                    )}
                  </div>
                )}
              </div>
